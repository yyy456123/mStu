...
<table border="1" cellspacing="0" align="center" class="con-b">
    ...
    <c:forEach items="${arr}" var="stuTask">
        <tr>
            ...
            <c:if test="${session_person.getUserIdentify()==0}">
                <td class="lf">
                    <button onclick="doAnswer('${stuTask.getTaskAccount()}','${stuTask.getIsFinish()}')">点击答题
                    </button>
                </td>
            </c:if>
            <c:if test="${session_person.getUserIdentify()==1}">
                <td class="lf">
                    <button onclick="doFenshu('${stuTask.getTaskAccount()}',${stuTask.getStudentAccount()},'${stuTask.getIsFinish()}')">
                        点击批改
                    </button>
                </td>
            </c:if>
            <td><img src="del.png"
                     onclick="delThisTask('${stuTask.getTaskAccount()}','${stuTask.getStudentAccount()}',${stuTask.getIsFinish()})">
            </td>
...
<script>
    ...
    function delThisTask(taskAccount, studentAccount, isFinish) {
        ...
        alert("删除 题目" + taskAccount + "  学生:" + studentAccount)
    }
